...
const DailyCheckUp: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'characters' }, (payload) => {
                setMainChars(prev => prev.map(c => c.id === (payload.new as any).id ? { ...c, ...payload.new } : c));
            })
    ...
    const rows = useMemo(() => {
        ...
        try {
            return accounts.map(acc => {
                ...
                // Build unlock states from character booleans and account booleans
    ...
    }, [accounts, mainChars, allItems, allChars, activeEvents, indexedEventProgress, todayStr]);
        ...
        groupedDailyUnlocks.forEach(group => {
            if (SEQUENTIAL_UNLOCK_GROUPS.has(group.label)) {
                group.items.forEach((item, idx) => {
                    map.set(item.key, idx > 0 ? group.items[idx - 1].key : null);
                });
            } else {
    ...
    const handleToggleUnlock = async (def: UnlockDef, accountId: string, charId: string | undefined, value: boolean) => {
        if (def.level === 'account') {
            ...
            try {
                ...
                console.error('Error toggling account unlock:', error);
                setAccounts(prev => prev.map(a => a.id === accountId ? { ...a, [def.key]: !value } : a));
            }
        } else if (charId) {
            ...
            try {
                ...
                console.error('Error toggling character unlock:', error);
                setMainChars(prev => prev.map(c => c.id === charId ? { ...c, [def.key]: !value } : c));
            }
        }
    };
            ...
            <Header
                ...
                actions={
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                        {!showItems ? (
                            <Button variant="secondary" size="sm" onClick={() => setShowItems(true)}>
                                Show Items
                            </Button>
                        ) : (
                            <>
                                <Button variant="primary" size="sm" onClick={() => { setShowItems(false); setActiveItemFilters(new Set(['for_sale'])); }}>
                                    Hide Items
                                </Button>
                                <Button variant={activeItemFilters.has('for_sale') ? "primary" : "secondary"} size="sm" onClick={() => toggleItemFilter('for_sale')}>
                                    For Sale
                                </Button>
                                <Button variant={activeItemFilters.has('in_stock') ? "primary" : "secondary"} size="sm" onClick={() => toggleItemFilter('in_stock')}>
                                    In Stock
                                </Button>
                                <Button variant={activeItemFilters.has('bulk') ? "primary" : "secondary"} size="sm" onClick={() => toggleItemFilter('bulk')}>
                                    Bulk
                                </Button>
                            </>
                        )}
                                ...
                                {groupedDailyUnlocks.map(group =>
                                    group.items.map((item: any) => {
                                        const count = item.isEvent
                                            ...
                                            : { completed: unlockProgressCounts[item.key] || 0, total: accounts.length };
